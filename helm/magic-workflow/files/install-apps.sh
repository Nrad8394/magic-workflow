...
mkdir -p "$APPS"
# Air-gapped: if pre-staged tarballs exist here they're used instead of GitHub.
# (Populate with scripts/fetch-nc-apps.sh; mounted at /apps-offline by compose.)
OFFLINE_DIR="${OFFLINE_DIR:-/apps-offline}"
...
install_app() {  # name  tarball-url
  ...
  if [ ! -e "$APPS/$name/appinfo/info.xml" ]; then
    # 1) Prefer a pre-staged offline tarball (air-gapped installs).
    if [ -f "$OFFLINE_DIR/$name.tar.gz" ]; then
      echo "[nc-apps] installing $name from offline bundle ..."
      if tar -xzf "$OFFLINE_DIR/$name.tar.gz" -C "$APPS"; then
        [ "$(id -u)" = "0" ] && chown -R www-data:www-data "$APPS/$name"
      else
        echo "[nc-apps] !! extract failed (offline): $name"; return 0
      fi
    else
      # 2) Fall back to the GitHub release tarball.
      ...
      if curl -fsSL "$url" -o "$tmp"; then
        ...
        echo "[nc-apps] !! could not download $name (no internet to GitHub? pre-stage with fetch-nc-apps.sh) — skipping"
        rm -f "$tmp"; return 0
      fi
      rm -f "$tmp"
    fi
